...
**게시된 모든 릴리스가 이 규칙을 따릅니다.** 규칙 도입 전에 올라간 `v1.0.0`도
2026-08-08에 같은 형식으로 이름을 바꿨습니다 — 재업로드가 아니라 이름만 바꾼 것이라
파일 내용과 SHA-256은 그대로입니다. 옛 이름(`DAISY_VAC_E_Front.hex`)으로 된
다운로드 주소는 더 이상 열리지 않습니다.
...
이름 규칙이 나중에 또 바뀌어도 스크립트를 고칠 필요가 없습니다.
